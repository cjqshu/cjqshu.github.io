...
# https://github.com/jsyoon0823/GANITE/blob/master/ganite.py
# https://github.com/pytorch/pytorch/blob/9b2e453e246ea44f2071dfdac86ec4b0037a51a5/torch/autograd/functional.py
"""
...
def ganite (train_x, train_t, train_y, test_x, parameters):
  ...
  Args (Inputs):
      ...
      test_x (tuple of Tensors or Tensor): features in testing data

      parameters: GANITE network parameters
      - h_dim (int, optional): hidden dimensions
      - batch_size (int, optional): the number of samples in each batch
      - iterations (int, optional): the number of iterations for training
      - alpha (float, optional): hyper-parameter to adjust the loss importance
